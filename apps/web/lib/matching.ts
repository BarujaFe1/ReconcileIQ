...
/** Lightweight token_sort_ratio approximation for browser demo. */
...
function severity(impact: number): ExceptionSeverity {
...
function confidenceExact(amountDelta: number, feeDelta: number): number {
  let score = 98;
...
function confidenceFuzzy(nameScore: number, refScore: number, amountDelta: number): number {
...
function pushAudit(action: string, entityType: string, entityId: string, details: Record<string, unknown>, actor = "system") {
  auditTrail = [
    {
...
export function runClientReconciliation(): ReconciliationResponse {
  const feeByPayment = new Map(DEMO_FEES.map((f) => [norm(f.payment_ref), f]));
  const paymentsByRef = new Map(DEMO_PAYMENTS.map((p) => [norm(p.order_ref), p]));
  const usedPayments = new Set<string>();
  const matches: MatchCandidate[] = [];

  for (const order of DEMO_ORDERS) {
    const ref = norm(order.external_ref);
    ...
    if (!payment) {
      let bestScore = -1;
      let best: { payment: PaymentRow; nameScore: number; refScore: number } | null = null;
    ...
    const fee: FeeRow | undefined = feeByPayment.get(norm(payment.payment_id));
    const expectedGross = payment.net_amount + payment.fee_amount;
    const amountDelta = Math.round((order.gross_amount - expectedGross) * 100) / 100;
    ...
    if (method === "exact_ref") {
      ...
      status = Math.abs(amountDelta) < 0.5 && Math.abs(feeDelta) < 0.5 ? "matched" : "exception";
      reasons.push("Exact external_ref ↔ order_ref");
    } else {
      confidence = confidenceFuzzy(nameScore, refScore, amountDelta);
      status =
        Math.abs(amountDelta) < 1 && confidence >= 75 ? "fuzzy_matched" : "exception";
    }

    if (Math.abs(amountDelta) >= 0.5) reasons.push(`Amount delta R$ ${amountDelta.toFixed(2)}`);
    if (Math.abs(feeDelta) >= 0.5) reasons.push(`Fee anomaly R$ ${feeDelta.toFixed(2)}`);
    if (reasons.length === 1 || (method !== "exact_ref" && reasons.length === 2 && Math.abs(amountDelta) < 0.5 && Math.abs(feeDelta) < 0.5)) {
      if (!reasons.some((r) => r.startsWith("Amount") || r.startsWith("Fee") || r.startsWith("No "))) {
        // keep fuzzy reasons; add clean note when deltas are fine
      }
    }
    if (!reasons.some((r) => r.startsWith("Amount") || r.startsWith("Fee") || r.startsWith("No "))) {
      if (method === "exact_ref" || status === "fuzzy_matched") {
        reasons.push("Amounts and fees within tolerance");
      }
    }

    const impact = Math.round((Math.abs(amountDelta) + Math.abs(feeDelta)) * 100) / 100;
    if ((status === "matched" || status === "fuzzy_matched") && impact >= 0.5) {
      status = "exception";
    }
    ...
    matches.push({
      ...
      fee_delta: feeDelta,
      reasons,
      severity: status === "exception" ? severity(impact) : "low",
  ...
  const leakage = Math.round(exceptions.reduce((s, e) => s + e.financial_impact, 0) * 100) / 100;
  const feeAnomaly =
    Math.round(
      matches.filter((m) => Math.abs(m.fee_delta) >= 0.5).reduce((s, m) => s + Math.abs(m.fee_delta), 0) * 100,
    ) / 100;
  ...
  const matchedVolume = Math.round(
  ...
  return {
    summary: {
      ...
      unmatched_orders: matches.filter((m) => m.status === "unmatched_order").length,
      unmatched_payments: matches.filter((m) => m.status === "unmatched_payment").length,
...
): { exception_id: string; status: ExceptionItem["status"]; action: string; note: string; actor: string; timestamp: string } {
  const status: ExceptionItem["status"] =
    action === "confirm_match" || action === "reject_match" || action === "write_off"
      ? "resolved"
      : "investigating";
